				  ...
				  struct drm_dp_aux_msg *msg)
{
	...
	if (msg->request == DP_AUX_NATIVE_WRITE) {
		...
		for (i = 0; i < msg->size; ++i) {
			ret = cdns_mhdp_dpcd_write(&mhdp->mbox,
						   msg->address + i, buf[i]);
			if (!ret)
				continue;
		...
		ret = cdns_mhdp_dpcd_read(&mhdp->mbox, msg->address,
					  msg->buffer, msg->size);
		if (ret) {
			dev_err(mhdp->dev,
...
static int cdns_mhdp_link_training_init(struct cdns_mhdp_device *mhdp)
{
	...
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DPTX_PHY_CONFIG, reg32);

	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_ENHNCD,
			    mhdp->sink.enhanced & mhdp->host.enhanced);

	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_LANE_EN,
	...
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DPTX_PHY_CONFIG,
					       ...
					       unsigned int training_interval)
{
	...
	do {
		...
		cdns_mhdp_adjust_lt(&mhdp->mbox, mhdp->link.num_lanes,
...
static bool cdns_mhdp_link_training_cr(struct cdns_mhdp_device *mhdp)
{
	...
	do {
		...
		cdns_mhdp_adjust_lt(&mhdp->mbox, mhdp->link.num_lanes, 100,
				   ...
				   unsigned int training_interval)
{
	...
	ret = cdns_mhdp_reg_read(&mhdp->mbox, CDNS_DP_FRAMER_GLOBAL_CONFIG, &reg32);
	if (ret < 0) {
		dev_err(mhdp->dev,
	...
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_FRAMER_GLOBAL_CONFIG, reg32);
...
static int cdns_mhdp_link_up(struct cdns_mhdp_device *mhdp)
{
	...
	err = cdns_mhdp_reg_read(&mhdp->mbox, CDNS_DP_FRAMER_GLOBAL_CONFIG, &resp);
	if (err < 0) {
		dev_err(mhdp->dev,
			"Failed to read CDNS_DP_FRAMER_GLOBAL_CONFIG %d\n",
			err);
	...
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_FRAMER_GLOBAL_CONFIG, resp);
				      ...
				      const struct drm_display_mode *mode)
{
	...
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_BND_HSYNC2VSYNC(stream_id),
			    bnd_hsync2vsync);
	...
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_HSYNC2VSYNC_POL_CTRL(stream_id),
			    hsync2vsync_pol_ctrl);

	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_FRAMER_PXL_REPR(stream_id),
			    pxl_repr);
	...
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_FRAMER_SP(stream_id),
			    dp_framer_sp);

	front_porch = mode->crtc_hsync_start - mode->crtc_hdisplay;
	back_porch = mode->crtc_htotal - mode->crtc_hsync_end;
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_FRONT_BACK_PORCH(stream_id),
			    CDNS_DP_FRONT_PORCH(front_porch) |
			    CDNS_DP_BACK_PORCH(back_porch));

	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_BYTE_COUNT(stream_id),
			    mode->crtc_hdisplay * bpp / 8);

	msa_h0 = mode->crtc_htotal - mode->crtc_hsync_start;
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_MSA_HORIZONTAL_0(stream_id),
	...
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_MSA_HORIZONTAL_1(stream_id),
			    msa_horizontal_1);

	msa_v0 = mode->crtc_vtotal - mode->crtc_vsync_start;
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_MSA_VERTICAL_0(stream_id),
	...
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_MSA_VERTICAL_1(stream_id),
			    msa_vertical_1);
	...
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_MSA_MISC(stream_id),
			    misc0 | (misc1 << 8));

	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_HORIZONTAL(stream_id),
			    CDNS_DP_H_HSYNC_WIDTH(hsync) |
			    CDNS_DP_H_H_TOTAL(mode->crtc_hdisplay));

	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_VERTICAL_0(stream_id),
	...
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_VERTICAL_1(stream_id),
			    dp_vertical_1);

	cdns_mhdp_reg_write_bit(&mhdp->mbox, CDNS_DP_VB_ID(stream_id), 2, 1,
				(mode->flags & DRM_MODE_FLAG_INTERLACE) ?
				CDNS_DP_VB_ID_INTERLACED : 0);

	ret = cdns_mhdp_reg_read(&mhdp->mbox, CDNS_DP_FRAMER_GLOBAL_CONFIG,
				 &framer);
	if (ret < 0) {
		dev_err(mhdp->dev,
			"Failed to read CDNS_DP_FRAMER_GLOBAL_CONFIG %d\n",
	...
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_FRAMER_GLOBAL_CONFIG, framer);
}
				 ...
				 const struct drm_display_mode *mode)
{
	...
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_LINE_THRESH(0),
			    line_thresh & GENMASK(5, 0));

	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DP_STREAM_CONFIG_2(0),
				    ...
				    struct drm_bridge_state *bridge_state)
{
	...
	ret = cdns_mhdp_reg_read(&mhdp->mbox, CDNS_DPTX_CAR, &resp);
	if (ret < 0) {
		dev_err(mhdp->dev, "Failed to read CDNS_DPTX_CAR %d\n", ret);
		goto out;
	}

	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DPTX_CAR,
				     ...
				     struct drm_bridge_state *bridge_state)
{
	...
	cdns_mhdp_reg_read(&mhdp->mbox, CDNS_DP_FRAMER_GLOBAL_CONFIG, &resp);
	resp &= ~CDNS_DP_FRAMER_EN;
	...
	cdns_mhdp_reg_read(&mhdp->mbox, CDNS_DPTX_CAR, &resp);
	cdns_mhdp_reg_write(&mhdp->mbox, CDNS_DPTX_CAR,
			    resp & ~(CDNS_VIF_CLK_EN | CDNS_VIF_CLK_RSTN));
...
static bool cdns_mhdp_detect_hpd(struct cdns_mhdp_device *mhdp, bool *hpd_pulse)
{
	...
	*hpd_pulse = false;
	...
	hpd_event = cdns_mhdp_read_hpd_event(&mhdp->mbox);
	...
	hpd_status = cdns_mhdp_get_hpd_status(&mhdp->mbox);
	if (hpd_status < 0) {
